...
class TMC5160:
    def __init__(self, config):
        ...
        globalscaler = self.fields.get_field("globalscaler")       
        logging.info("globalscaler=%#x", globalscaler)
        
        irun = self.fields.get_field("irun")       
        logging.info("irun=%#x", irun)        

        ihold = self.fields.get_field("ihold")       
        logging.info("ihold=%#x", ihold)  
        
        sg_result = self.fields.get_field("sg_result")       
        logging.info("sg_result=%#x", sg_result)
                
        s2vsa = self.fields.get_field("s2vsa")       
        logging.info("s2vsa=%#x", s2vsa) 
               
        s2vsb = self.fields.get_field("s2vsb")       
        logging.info("s2vsb=%#x", s2vsb) 
         
        stealth = self.fields.get_field("stealth")       
        logging.info("stealth=%#x", stealth)
        
        fsactive = self.fields.get_field("fsactive")       
        logging.info("fsactive=%#x", fsactive) 
        
        csactual = self.fields.get_field("csactual")       
        logging.info("csactual=%#x", csactual)
            
        stallguard = self.fields.get_field("stallguard")       
        logging.info("stallguard=%#x", stallguard)
        
        ot = self.fields.get_field("ot")       
        logging.info("ot=%#x", ot)
          
        otpw = self.fields.get_field("otpw")       
        logging.info("otpw=%#x", otpw)
                                    
        s2ga = self.fields.get_field("s2ga")       
        logging.info("s2ga=%#x", s2ga)
          
        s2gb = self.fields.get_field("s2gb")       
        logging.info("s2gb=%#x", s2gb)
        
        ola = self.fields.get_field("ola")       
        logging.info("ola=%#x", ola)
                
        olb = self.fields.get_field("olb")       
        logging.info("olb=%#x", olb)
          
        stst = self.fields.get_field("stst")       
        logging.info("stst=%#x", stst)                         
